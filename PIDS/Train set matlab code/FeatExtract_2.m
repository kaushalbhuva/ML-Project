...
for i = 1:126
%% File name creator
    train = ['s' num2str(i) '.xlsx'];

%% Reading a data file

dat = importdata(train);
...
% Assign labels
if i <= 52   % 1 to 52 idle samples =1
    label = 1;
elseif 53 <= i && i <= 78 %% ||  49 <= i && i <= 54 %% 53 to 78 ladder = 2
    label = 2;
elseif 79 <= i && i <= 112  %%  stone =3
    label = 3;
elseif 113 <= i && i <= 126  %% wire cut = 4
    label = 4;
end
...
dlmwrite(['./train/' 'train.csv'],FeatVec,'delimiter',',','-append')
end
